...
在本次 EmiyaOJ-Cloud 项目中，我作为架构与核心后端负责人，主导了整个系统的技术选型和架构设计。在项目启动阶段，我对比了 Spring Boot 单体架构与 Spring Cloud 微服务架构的利弊——单体架构虽然开发速度快、部署简单，但随着功能模块增多，代码耦合会越来越严重，一个模块的故障可能影响整个系统，且五人团队并行开发时 Git 冲突频繁。最终我选择了 Spring Cloud 2025.0.0 + Spring Cloud Alibaba 2025.0.0.0 微服务全家桶方案，按业务域将系统拆分为 Auth（认证授权）、Problem（题目竞赛）、Judge（判题提交）、Blog（博客社区）、Chat（AI 问答）和 Moderation（内容审核）六个独立微服务，外加 Gateway 统一网关和 Common 公共模块，共计八个 Maven 子模块。

在 Maven 父工程搭建中，我在根 `pom.xml` 中通过 `<dependencyManagement>` 统一锁定了 Spring Boot 3.5.5、Spring Cloud 2025.0.0、MyBatis-Plus 3.5.16、JJWT 0.12.6 等核心依赖版本，彻底避免了子模块间的版本冲突。我为每个微服务制定了统一的三模块内部结构规范——`*-api` 存放 Feign 接口定义、`*-dto` 存放数据传输对象、`*-service` 存放业务逻辑和数据库访问——这一规范在所有六个业务服务中保持一致，团队成员无论切换到哪个服务都能快速定位代码。在 Common 模块中，我封装了 `ResponseResult<T>` 统一响应体（含 `code`、`message`、`data` 和 `success` 字段）、`PageVO<T>` 分页封装、`GlobalExceptionHandler` 全局异常处理器（将 `BadRequestException`、`CustomerAuthenticationException` 等自定义异常自动转换为标准 JSON 响应），以及 `JwtUtil`、`RedisUtil`、`BaseContext` 等通用工具类。这些基础设施的提前沉淀使得其他成员在开发业务代码时无需关心横切关注点，显著提升了整体开发效率。

在数据库设计上，我提出了"按业务域分库、服务内直连、跨域接口调用"的核心原则——认证、题目、判题和博客四个业务域各自独立数据库，每个微服务仅直连自身数据库，跨域数据访问必须通过 Feign 或消息队列接口完成。我主导编写了四套数据库的完整 DDL 脚本（`emiya_oj_auth.sql`、`emiya_oj_problem.sql`、`emiya_oj_judge.sql`、`emiya_oj_blog.sql`），涵盖 28 张核心业务表，所有表统一采用 `utf8mb4` 字符集、逻辑删除标记（`deleted` 字段）和审计字段（`create_time`、`update_time`、`create_by`、`update_by`），并整理了 `emiya-oj_final.sql` 演示数据初始化脚本供答辩使用。ID 生成策略上，我根据表的特性选用了两种方案——高频写入的 `submission` 表使用 MyBatis-Plus 的 `ASSIGN_ID`（雪花算法）以支持分布式场景，而低频维护的 `role`、`permission` 等配置表使用 `AUTO_INCREMENT`。

##### 认证网关与 RBAC 权限体系开发

我独立设计并实现了 Gateway 与 Auth Service 协作的完整认证授权链路。Gateway 中的 `AuthGlobalFilter` 全局过滤器是整个安全体系的核心——它实现了 `GlobalFilter` 和 `Ordered` 接口，在请求路由到业务服务之前对所有非白名单请求进行拦截。过滤器的处理流程分为五步：第一步，从 `Authorization` 请求头提取 Bearer Token；第二步，利用 Common 模块的 `JwtUtil.parseJWT()` 以 HMAC-SHA256 算法本地解析 JWT，提取其中的 `userId`、`username` 和 `permissions` 声明；第三步，通过 Redis 查询 `token_{userId}` 键验证 Token 是否仍存在于白名单中（若不存在说明用户已登出或 Token 被管理员撤销）；第四步，刷新 Redis 中 Token 的 TTL（7200 秒）实现滑动过期；第五步，通过 `ServerWebExchange.mutate().request().header()` 将 `X-User-Id`、`X-User-Name`、`X-User-Roles` 三个请求头注入到下游请求中。这种"JWT 无状态认证 + Redis 白名单实现主动登出"的设计，既保留了 JWT 不依赖服务端存储的高性能优势，又解决了 JWT 自身无法主动失效的安全痛点——用户登出时只需删除 Redis 中对应的白名单记录，Token 即刻失效。

在权限管理方面，我设计并实现了菜单/按钮/接口三级 RBAC 权限模型。核心三张表为 `user`、`role` 和 `permission`，通过 `user_role` 和 `role_permission` 两张中间表维护多对多关系。`permission` 表通过 `parent_id` 字段自关联实现树形层级结构，`permission_type` 字段区分三种权限类型：1-菜单（控制管理端左侧导航可见性）、2-按钮（控制页面内新增/编辑/删除等操作按钮的显隐）、3-接口（控制后端 API 的访问权限）。用户登录成功后，Auth Service 通过三表 JOIN 查询汇总当前用户的所有权限编码集合，将其写入 JWT Claims。管理端前端根据权限编码集合递归渲染菜单树——从顶级菜单（`parent_id IS NULL`）开始遍历，对每个节点检查其 `permission_code` 是否在用户权限集合中，若在则渲染该菜单项并继续递归处理其子节点。Auth Service 对外暴露了 50 余个 REST API，涵盖用户管理的完整 CRUD、角色分配、权限绑定、密码重置、状态切换和批量操作，以及角色管理和权限树的增删改查与状态管理，所有接口均遵循统一的 `ResponseResult<T>` 响应格式。

##### 判题模块核心开发与 Go-Judge 沙箱集成

我独立负责了 Judge Service 的全栈后端开发，这是整个系统中技术复杂度最高的模块。判题链路的整体架构设计遵循"提交与判题异步解耦"的核心原则——`JudgeController.submitCode()` 接口在接收到用户提交后，仅执行同步校验（题目存在性、语言启用状态、竞赛资格四重校验）并创建 PENDING 状态的 `submission` 记录后立即返回提交编号，整个同步过程控制在 500ms 以内；耗时操作通过 Spring `@Async` 注解移交到 `JudgeExecutor` 的异步线程池中执行。

在判题执行引擎 `JudgeExecutor.executeJudgeAsync()` 中，我设计了完整的异步判题流水线：第一步，通过 `ProblemFeignClient` 从 Problem Service 获取题目时间/内存限制、语言编译运行命令模板和全部测试用例（含隐藏用例）；第二步，判断语言类型——编译型语言先调用 `GoJudgeService.compile()` 编译源代码，解释型语言直接跳过编译阶段；第三步，遍历全部测试用例，对每个用例调用 `GoJudgeService.run()` 在 Go-Judge 沙箱中执行；第四步，对每个用例的输出进行严格比对（trim 首尾空白后字符串匹配），记录单用例耗时和内存；第五步，调用 `JudgeResultCalculator` 汇总计算最终判题状态并保存到 `submission_judge_result` 和 `submission_case_result` 表。

Go-Judge 沙箱的集成是判题模块中技术难度最大的环节。Go-Judge 是一个独立的 Docker 容器服务，通过 REST API（`POST /api/judge`）接受编译和运行请求。我在 `GoJudgeService` 中封装了与 Go-Judge 的 HTTP 通信细节，包括：利用 `LanguageCommandBuilder` 将 `language` 表中的编译命令模板（如 `g++ -O2 -std=c++17 {SourceFileName} -o {ExecutableFileName}`）和运行命令模板（如 `./{ExecutableFileName}`）中的占位符替换为实际文件名；构造 `GoJudgeRequest` 对象，设置 `Cmd` 结构中的 `args`（命令参数列表）、`env`（环境变量）、`copyIn`（通过 Base64 编码传入源代码文件）、`copyOut`（指定收集 stdout 和 stderr）以及 `cpuLimit`（纳秒级 CPU 时间限制）、`memoryLimit`（字节级内存限制）、`procLimit`（进程数限制）等多维资源约束；编译完成后提取 Go-Judge 返回的 `fileIds`（编译产物在沙箱中的文件 ID），在后续运行步骤中通过 `copyIn` 引用这些文件 ID，实现编译产物在不同命令间的传递。

在判题状态机设计上，我定义了从 PENDING(0)、JUDGING(1) 到 AC(2)、CE(3)、SE(4)、WA(5)、TLE(6)、MLE(7)、RE(8)、OLE(9)、PA(10) 共 11 种状态。状态汇总算法遵循以下优先级逻辑：若编译失败 → CE；若沙箱不可用或测试用例为空 → SE；遍历用例过程中，每个用例独立判定——Go-Judge 返回 `TIME_LIMIT_EXCEEDED` → 该用例 TLE、`MEMORY_LIMIT_EXCEEDED` → MLE、`NONZERO_EXIT_STATUS` 或 `SIGNALLED` → RE、`OUTPUT_LIMIT_EXCEEDED` → OLE；全部用例执行完毕后，若所有用例通过 → AC，部分通过 → PA，全部未通过则取第一个失败用例的状态作为最终状态。提交查询接口实现了按权限过滤——普通用户只能查看自己的完整代码和单用例详情，查看他人提交时隐藏源代码和隐藏用例的具体输入输出；管理员可查看全站提交记录。

##### CI/CD 流水线与容器化部署

我主导了项目从开发环境到演示环境的完整部署方案设计。在容器化方面，我编写了完整的 `docker-compose.yml` 编排文件，定义了 13 个服务容器（MySQL、Redis、Nacos、RabbitMQ、MinIO、Go-Judge 六大基础设施 + Gateway、Auth、Problem、Judge、Blog、Chat、Moderation 七个业务服务）。在编排设计中，我重点解决了三个关键问题：第一是服务启动顺序——基础设施容器配置了 `healthcheck`（如 MySQL 的 `mysqladmin ping`、Redis 的 `redis-cli ping`、Nacos 的 `curl /nacos/v1/console/health/readiness`），业务服务通过 `depends_on` 配合 `condition: service_healthy` 确保在依赖的基础设施就绪后才启动；第二是容器间通信——所有容器加入自定义桥接网络 `emiyaoj-network`，业务服务通过容器名（如 `mysql:3306`、`redis:6379`、`nacos:8848`）访问基础设施，无需暴露不必要的端口到宿主机；第三是数据持久化——为 MySQL、Redis、RabbitMQ、MinIO 配置了命名数据卷（`mysql-data`、`redis-data`、`rabbitmq-data`、`minio-data`），确保容器重启后数据不丢失。Go-Judge 容器单独配置了 `privileged: true` 和 `shm_size: 256m`，确保在 Docker Desktop 环境下也能正常运行资源限制功能。

在 CI/CD 方面，我独立设计并实现了 Jenkins 声明式流水线。流水线脚本采用"Pipeline as Code"理念编写，存储在项目仓库根目录的 `Jenkinsfile` 中，实现了配置与代码版本同步。流水线分为六个 Stage：Stage 1（Checkout）拉取后端仓库代码；Stage 2（Maven Build）执行 `mvn clean package -DskipTests` 构建所有模块；Stage 3（Unit Test）执行 `mvn test` 并收集 surefire 测试报告；Stage 4（Docker Build）为七个业务服务分别构建基于 `eclipse-temurin:21-jre-alpine` 的 Docker 镜像并推送到镜像仓库；Stage 5（Deploy）通过 SSH 登录部署服务器，执行 `docker compose down` 停止旧容器、`docker compose pull` 拉取新镜像、`docker compose up -d` 启动新容器；Stage 6（Health Check）等待 30 秒后逐项检查——`docker ps` 验证 13 个容器全部 Running、`curl` Nacos 控制台确认服务注册状态、`curl Gateway /actuator/health` 验证网关可达。敏感配置（数据库密码、JWT 签名密钥、AI API Key、阿里云 AK/SK）全部通过 Jenkins Credentials 管理，流水线执行时通过 `credentials()` 方法注入环境变量，全程不在控制台或日志中明文显示。流水线参数化支持选择构建分支（main/develop）和部署目标（local/demo-server），灵活适配不同演示场景。

此外，我负责整理了项目全部的 SQL 初始化脚本——将分散在各模块的建表语句统一汇总到 `sql/` 目录下，按数据库分为四个核心脚本，外加语言配置、竞赛、题单和权限初始化数据脚本，并在 `docker-compose.yml` 中配置 MySQL 容器的 `docker-entrypoint-initdb.d` 自动加载机制，实现首次启动即自动建库建表。在文档方面，我参与编写了《需求规格说明书》中的功能需求和接口需求章节、《概要设计说明书》中的总体结构设计、处理流程和系统部署方案章节，独立编写了认证网关模块和判题提交模块的详细设计说明书，并为团队提供了 Swagger UI 使用指南和 Apifox 接口调试指引。
...
在小组沟通配合方面，由于我同时负责 Gateway、Auth、Problem 和 Judge 四个核心模块，与其他成员的接口依赖关系最为密集——Judge 需要调用 Problem 的 Feign 接口获取判题数据（题目限制、语言命令、测试用例），Gateway 需要调用 Auth 的 Feign 接口解析 Token 获取用户权限，Blog 模块需要参考 Auth 的用户上下文设计来获取当前登录用户信息。在开发过程中，虽然我在 Day2 就提前定义了 Feign 接口签名和 DTO 结构并写入 Swagger 文档，但部分接口在实现过程中因实际业务需求变化进行了调整——例如判题数据获取接口最初只返回 `problemId` 对应的测试用例，后来因竞赛判题需要增加了 `contestId` 参数用于竞赛提交校验；又如 Gateway 注入下游的请求头从最初的 `X-User-Id` 扩展为 `X-User-Id`、`X-User-Name`、`X-User-Roles` 三个头。这些变更未能第一时间同步更新文档，导致前端曹馨悦在联调时发现 Auth 接口返回的字段与预期不符，Blog 模块蔡兆炫也需要额外确认用户上下文中是否包含角色信息。这提醒我，在微服务架构中，接口即契约——接口定义的任何变更都应遵循"先更新文档，再修改代码"的纪律。后续项目应在 CI 流水线中集成 Swagger 文档自动生成和发布步骤，确保文档与代码始终保持同步。

另一个沟通上的教训是数据库初始化脚本的协作方式。我作为数据库脚本的整理者，在 Day3 完成了四套数据库的 DDL 编写，但 Blog 模块在 Day5 新增了 `blog_picture` 图片元数据表和 `blog_tag` 标签表，Moderation 模块在 Day5 新增了博客审核相关字段（`audit_status`、`audit_task_id`、`audit_reason`），这些变更通过口头沟通告知我后，我手动合并到统一的 SQL 脚本中。这种方式在变更频繁时效率低下且容易遗漏。后续项目应将 SQL 脚本也纳入 Git 版本管理，各模块负责人在自己的分支上维护本模块的 DDL 变更，合并时由架构负责人统一审核，确保数据库结构的变更可追溯、可回滚。

在技术难点攻克方面，Go-Judge 沙箱在 Windows Docker Desktop 环境下的配置是判题模块开发中遇到的最大技术障碍。Go-Judge 需要在容器内创建 cgroup 来实现 CPU 和内存的资源限制，这要求容器具备 `privileged` 权限和足够的共享内存（`shm_size`）。在团队成员的 Windows 开发环境中，Docker Desktop 版本从 4.20 到 4.35 不等，部分旧版本对 `--shm-size` 参数的支持不完整，导致 Go-Judge 容器启动后执行判题时返回 `INTERNAL_ERROR`。排查过程花费了近半天时间——最初怀疑是 Go-Judge 镜像版本问题，尝试了多个镜像标签；然后怀疑是 Docker Desktop 的 WSL2 后端配置问题，检查了 `.wslconfig` 的内存和交换文件设置；最终通过对比不同成员的 Docker Desktop 版本和 `docker inspect` 容器的详细信息，定位到共享内存未正确分配。解决方案是在 `docker-compose.yml` 中为 Go-Judge 容器明确配置 `shm_size: '256m'`，并统一团队使用 Docker Desktop 4.30 以上版本。这一经历让我深刻意识到容器化环境下基础设施一致性的重要性——团队应在项目启动时就统一开发环境版本，并通过 `docker-compose.yml` 固化所有配置参数，避免"在我的机器上能跑"的问题。

判题状态机的边界情况处理也有改进空间。当前 Go-Judge 返回 `INTERNAL_ERROR` 状态时，代码统一将其标记为 SE（System Error），但 `INTERNAL_ERROR` 可能来源于多种原因：沙箱自身故障（如 cgroup 不可用）、测试用例配置错误（如 input 字段为空导致程序异常读取）、用户代码触发了沙箱的保护机制（如访问了受限的系统调用）。统一标记为 SE 虽然简化了前端展示，但对用户排查问题帮助有限——用户看到 SE 时无法判断是自己的代码问题还是系统问题。后续应在 `GoJudgeService` 中细化 `INTERNAL_ERROR` 的错误分类，结合 Go-Judge 返回的 `error` 字段内容（如 "cgroup is not mounted"、"file not found" 等）区分具体原因，并在 `submission_judge_result` 的 `error_message` 中给出更明确的提示。

在 Maven 多模块项目构建方面，初始的父 `pom.xml` 中没有配置 `spring-boot-maven-plugin` 的 `classifier = 'exec'` 参数，导致 `mvn package` 后各服务打出的 jar 包缺少可执行入口，Docker 镜像构建时报 "no main manifest attribute" 错误。排查后发现在 Spring Boot 3.5.x 版本中，`spring-boot-maven-plugin` 的默认行为与旧版本有所不同，需要在父 POM 中统一配置 `exec` 分类器。这一小问题提醒我，即便是成熟框架的版本升级也可能引入行为变化，应在项目初期通过一个端到端的"Hello World"验证（从代码编写到 Docker 运行）来确认构建链路的完整性。

在进度把控方面，Day4-Day5 阶段我同时推进 Gateway、Auth、Problem 和 Judge 四个模块的开发，每天的工作量非常大。Gateway 的 `AuthGlobalFilter` 过滤器编写和 JWT 解析逻辑调试、Auth Service 的 50 余个 REST API 和 RBAC 权限汇总算法、Problem Service 的题目/测试用例/语言/竞赛全套 CRUD、Judge Service 的异步判题流水线和 Go-Judge 集成——这些任务在两天内完成编码确实过于紧张，导致部分边缘接口（如 Auth 中的批量删除用户、操作日志查询，Problem 中的标签颜色管理）的实现质量未达到预期，代码中存在较多的硬编码和重复逻辑。回顾起来，应该在 Day2 详细设计阶段就对接口按优先级排序——P0 为必须实现的核心链路接口（登录、题目 CRUD、提交判题、提交查询）、P1 为联调必需的接口（权限树查询、竞赛校验、语言列表）、P2 为锦上添花的接口（批量删除、操作日志、标签管理）。开发时先集中精力完成 P0 和 P1，确保核心链路可以走通，P2 接口放在 Day6-Day7 集成阶段补充。这样既能保证关键路径的开发质量，也能为联调争取更多时间。此外，在模块间存在强依赖的情况下（如 Judge 依赖 Problem 的 Feign 接口），应该约定先由被依赖方提供 Mock 实现或至少给出接口的请求示例和响应示例，让依赖方可以并行开发和自测，而不是等待被依赖方完全开发完毕才开始联调。

不足之处在于代码中部分异常处理仍使用了通用的 `RuntimeException`，未充分利用 Common 模块中定义的 `BadRequestException`（参数校验失败）、`CustomerAuthenticationException`（认证失败）和 `ForbiddenException`（权限不足）等业务异常类。例如 Judge Service 中提交校验失败时直接 `throw new RuntimeException("题目不存在")`，Gateway 应捕获此类异常并返回统一格式的错误 JSON，但实际上 `RuntimeException` 被 `GlobalExceptionHandler` 的兜底处理器捕获后统一返回了"系统繁忙，请稍后重试"的模糊提示，用户体验不佳。代码风格的一致性在多人协作项目中尤为重要——应建立代码审查（Code Review）环节，在合并 PR 前由至少一名其他成员检查异常处理、日志规范和命名风格。数据库事务管理方面，Judge Service 的判题结果保存涉及 `submission` 状态更新、`submission_case_result` 批量插入和 `submission_judge_result` 插入三次写操作，虽然业务上这三次操作应该在同一个事务中完成以保证数据一致性，但当前实现中未显式使用 `@Transactional` 注解控制事务边界。在极端情况下（如数据库连接中断），可能出现 `submission` 状态已更新为 JUDGING 但判题结果未写入的不一致状态。后续应在 `JudgeExecutor` 的异步方法上增加事务管理，并利用 MyBatis-Plus 的逻辑删除特性确保数据操作的原子性。在单元测试方面，我仅在 Auth 模块中为 JWT 工具类编写了几个基本的编解码测试用例，Judge 模块的判题状态汇总算法和 Go-Judge 请求构造逻辑均缺少自动化测试覆盖。判题模块是系统中最容易因边界情况导致 Bug 的模块（如不同语言命令模板的占位符替换、Go-Judge 返回状态的枚举映射、多用例结果的汇总逻辑），缺乏测试覆盖意味着每次修改判题逻辑都需要手动提交代码并观察结果，效率低且容易引入回归 Bug。后续应为核心判题逻辑补充单元测试——利用 Mockito 模拟 `ProblemFeignClient` 和 `GoJudgeService` 的返回值，验证 `JudgeExecutor` 在各种判题场景下的状态汇总结果是否正确。
