...
**Request Body** (`TestCaseSaveDTO`)：`id` 必填，其他字段为**完整更新**（采用 PUT 语义，所有字段均会被覆盖，建议传递完整对象）。
...
> 不过滤状态，直接返回语言详情，不存在时返回 `code=404`。
...
**Request Body** (`LanguageSaveDTO`)：`id` 必填，其他字段为**完整更新**（采用 PUT 语义，所有字段均会被覆盖，建议传递完整对象）。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | long | 测试用例 ID（更新时必填） |
| `problemId` | long | 关联题目 ID（新增时必填） |
| `input` | string | 输入数据 |
| `output` | string | 预期输出 |
| `isSample` | int | 是否为样例：0-否，1-是 |
| `score` | int | 分值 |
| `sortOrder` | int | 排序权重 |
